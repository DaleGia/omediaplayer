...
class usb_mounter:
    ...
    def usb_mount(self):
        ...
        for index, drive in enumerate(self._mount_mapping):
            # tests if USB drive is mounted
...
#                   print("is_usb_connected: " + str(is_usb_connected))
                   if(is_mounted > -1):
                      if(is_usb_connected > -1):
                          ...
                          self._mount_mapping[index] = (drive[0], drive[1], drive[2], True)
                      else:
                          print("Mount unsuccessful")
                          self._mount_mapping[index] = (drive[0], drive[1], drive[2], False)
                   else:
                       print("Mount unsuccessful")
                       self._mount_mapping[index] = (drive[0], drive[1], drive[2], False)
